...
def case_1fpy():
    test_dpa_values = np.geomspace(1e-05, 1e03, num=7)
    temperature_values = np.linspace(400, 1300, num=10)
    results_folder = "Results/parametric_studies/case_1fpy/transient/"
    cases = [
        # [1e-01, 400],
        [1e-01, 450],
        [1e-01, 500],
    ...
    for case in cases:
        dpa = case[0]
        T = case[1]
        print("Running case: T={:.0f} dpa={:.2e}".format(T, dpa))
        festim_sim(
            T=T,
            dpa=dpa,
            results_folder_name=results_folder + "dpa={:.2e}/T={:.0f}/".format(dpa, T),
            transient_run=True,
